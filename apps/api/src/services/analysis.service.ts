...
export class AnalysisService {
  ...
  async analyzeWallet(address: string, limit = 10): Promise<WalletAnalysisResponse> {
    ...
    const activity = this.toWalletActivity(address, transactions);
    const risk = scoreWalletActivity(activity);
    const cluster = aggregateClusterRisk({
      clusterId: `wallet:${address}`,
      walletScores: [risk],
      cycleScore: estimateCycleScore(transactions),
      fundingScore: estimateCommonFunderScore(transactions),
      temporalScore: estimateTemporalScore(transactions),
      inorganicVolumeShare: estimateInorganicShare(transactions),
    });
    ...
    return {
      ...
      graph,
      transactions,
      reasons: cluster.topReasons.length ? cluster.topReasons : ['transaction graph analyzed from Solana RPC'],
    };
  }
